class BowlingGame {
    ...
    void roll(int pins) {
        ...
        if (frames == LAST_FRAME) {
            if (pins != MAX_PINS) pendingPins = pins;
            return;
        }
        score += pins;
        if (pendingPins != null) {
            if (pendingPins + pins == MAX_PINS) countNext1++;
            pendingPins = null;
            frames++;
    ...
    int score() {
        if (!gameOver()) throw new IllegalStateException("Score cannot be taken until the end of the game");
        return score;
